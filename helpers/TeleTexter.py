...
def send_telegram_message(bot_token, chat_id, message):
    ...
    if response.json().get("ok") == True:
        print(
            f"Message sent successfully! -> message_id: {response.json().get('result').get('message_id')}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Send a message to a Telegram channel")
    ...
    parser.add_argument(
        "--spam", help="Send messages continuously", action="store_true"
    )
